...
DB: ./dtm.bolt
Bucket: global, branches, index
Key/Value:
	global: 	gid, TransGlobalStore
	branches: 	gid+index, TransBranchStore --- 利用cursor.Seek(prefix)可以遍历拥有特定前缀的键值对（前缀扫描）
	index:		timestamp-gid, gid --- cursor.Seek(min)使用可排序的时间编码可以遍历特定日期范围的数据（区间扫描）

BoltDB是一个纯golang编写的支持事务的文件型单机kv数据库。etcd使用boltdb作为存储引擎
